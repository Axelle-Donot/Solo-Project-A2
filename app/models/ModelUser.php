...
class ModelUser extends Model
{
    ...
    public static function getCartIdByUserId(): string|bool
    {
        $sql = "SELECT cart_id FROM proj__user u JOIN proj__shopping_cart s ON u.user_id = s.customer_id WHERE user_id =:id_tag";
        try {
            $req_prep = self::getPdo()->prepare($sql);
            $state = $req_prep->execute(array("id_tag" => Session::getUserId()));
            $req_prep->setFetchMode(PDO::FETCH_CLASS, 'ModelUser');
            $cart_id = $req_prep->fetch()->get('cart_id');
        } catch (PDOException $e) {
            if (Conf::getDebug()) echo $e->getMessage();
            return false;
        }
